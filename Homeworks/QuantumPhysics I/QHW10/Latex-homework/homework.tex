...
\begin{homeworkProblem}
    ...
    首先我们求解能量算符的本征值和本征态,其本征方程为：$|\hat{H}-H_nI|=0$，得到本征值为：$H_1=2;H_2=1,H_3=-1$单位为$\lambda_E$，其本征态为：
    $$
    \begin{aligned}
        \Ket{H_1} &= \Ket{3}\\
        \Ket{H_2} &= \Ket{1} + \Ket{2}\\
    ...
    因此可以讲表象转换矩阵写为：
    $$
    \ket{H_n}\bra{n}=\begin{pmatrix}
        0 & 0 & 1 \\
        1 & 1 & 0 \\
        -1 & 1 & 0
    \end{pmatrix}
    $$
我们从而可以写出哈密顿表象下的$\ket{\alpha}$态：$\ket{H_n} \braket{n|\alpha} = -i\ket{H_1} + (-2+i)\ket{H_2} - (2+i)\ket{H_3} $\\
同样的，写出哈密顿表象下的算符$$
\hat{B}_H=\ket{H_n}\braket{n|\hat{B}|n}\ket{H_n}=\ket{H_n}\braket{n|\beta}\braket{\beta|n}\bra{H_n}
$$
$$
\hat{B}_H=\begin{pmatrix}
    4 & -2i & 2i \\
    2i & 1 & -1 \\
    -2i & -1 & 1
\end{pmatrix}
