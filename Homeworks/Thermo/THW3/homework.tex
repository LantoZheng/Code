...
\begin{homeworkProblem}
    容积为$2.5\times10^{-3}m^3$的烧瓶内有$1.0\times10^{15}$个氩分子,有$4.0\times10^{15}$个氮分子和$3.3\times10^{-10}$kg的氩气。设混合气体的温度为$423$K,求混合气体的压强。已知氩气的摩尔质量为$M_{m,Ar}=40\times10^{-3}$kg/mol
    \textbf{Solution}
    已知：$V=2.5\times10^{-3}m^3$,$\nu_{O}=1.0\times10^{15} / N_a$
